...
class TrainingLogger:
    ...
    """
    def __init__(self, log_dir: str, run_number: int = None):
    ...
    def log(self, message: str, metrics: Dict[str, Any] = None):
        """
        ...
        if metrics is not None:
            # Convert non-serializable objects (like numpy floats) to Python floats for JSON
            serializable_metrics = {k: float(v) if hasattr(v, 'item') or isinstance(v, (float, int)) else v for k, v in metrics.items()}
