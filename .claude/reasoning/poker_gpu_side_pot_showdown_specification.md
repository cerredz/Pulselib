...
- GPU showdown payout will instead iterate through commitment layers, derive each side pot from `self.total_invested`, and award that layer only among eligible non-folded contributors.

What does not change:
- Hand ranking remains the current batched `HandRanks.dat` lookup path.
...
- Keep the existing vectorized showdown hand evaluation in `environments/Poker/PokerGPU.py` so all showdown hands are still ranked in a single tensor pass.
- Replace the one-shot whole-pot payout block in `resolve_terminated_games()` with side-pot distribution driven by `self.total_invested[showdown_games, :self.active_players]`.
- For each showdown game, sort unique positive commitment levels, compute each layer size as the delta from the previous level, and build a contributor mask `invested >= level`.
- Restrict winner selection per layer to seats that both contributed to that layer and are still eligible at showdown (`ACTIVE` or `ALLIN`).
- Award each layer by integer division plus deterministic remainder assignment to the first winning seat in table order. This mirrors CPU chip conservation and avoids dropping chips on odd splits.

Implementation tasks:
- Modify `environments/Poker/PokerGPU.py` inside `resolve_terminated_games()` to compute `invested`, `eligible_mask`, and `hand_ranks` once for all showdown games, then distribute side pots layer-by-layer per game. Why: this is the root-cause location where total investment is currently ignored. Downstream impact: `step()` in the same file will now receive corrected stack updates before it clears per-hand investment state. Potential issues: masking mistakes can accidentally include folded players or exclude short-stack winners; integer division can leak chips if remainder handling is omitted. Validation: run `pytest tests/poker/test_poker_gpu_side_pot_showdown.py -q` and `pytest tests/poker/test_poker_gpu_showdown.py -q`.
...
- Recomputing hand ranks inside each side-pot loop would add unnecessary work on a non-training path and raise the risk of shape/device mistakes.
...
- Side-pot distribution may use a small Python loop over showdown games and commitment tiers because this executes once per terminated hand, not inside the action-selection or training hot path, and there is no simpler built-in batched primitive for arbitrary side-pot tiers.
- No new tensor copies should move data off-device; use tensor masking/indexing in place where safe.

PyTorch best-practice cross-reference:
- Device placement: all new tensors must be created on `self.device`.
- Tensor operations: keep hand evaluation vectorized; restrict any Python iteration to per-game side-pot tiers after batched ranking is complete.
